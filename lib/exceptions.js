(function () { 'use strict';

var sys, JsonRpcError, ParseError, InvalidRequestError, MethodNotFoundError, InvalidParamsError;

require('sugar');

sys = require('sys');

// Add a serialize() method to the prototype chain to ensure that the
// serialization happens properly
JsonRpcError = function JsonRpcError(msg) {
    this.name    = 'JsonRpcError';
    this.code    = -32603;
    this.message = msg;
    this.data    = Array.prototype.slice.call(arguments).splice();
};

JsonRpcError.prototype.serialize = function serialize() {
    return JSON.stringify({
        name    : this.name,
        message : this.message,
        code    : this.code,
        data    : this.data
    });
}

ParseError = function ParseError(data) { 
    this.name    = 'ParseError';
    this.message = 'Unable to parse payload as a JSON.';
    this.code    = -32700;
    this.data    = data;
};

InvalidRequestError = function InvalidRequestError(data) {
    this.name    = 'InvalidRequestError';
    this.message = 'The request object is not a valid JSON-RPC 2.0 object.';
    this.code    = -32600;
    this.data    = data;
};

MethodNotFoundError = function MethodNotFoundError(data) {
    this.name    = 'MethodNotFoundError';
    this.message = 'The JSON-RPC method does not exist, or is an invalid one.';
    this.code    = -32601;
    this.data    = data;
};

InvalidParamsError  = function InvalidParamsError(data) {
    this.name    = 'InvalidParamsError';
    this.message = 'The JSON-RPC method\'s parameters are invalid.';
    this.code    = -32602;
    this.data    = data;
};

sys.inherits(Error, JsonRpcError);
sys.inherits(ParseError, JsonRpcError);
sys.inherits(InvalidRequestError, JsonRpcError);
sys.inherits(MethodNotFoundError, JsonRpcError);
sys.inherits(InvalidParamsError, JsonRpcError);

module.exports = {
    JsonRpcError        : JsonRpcError,
    ParseError          : ParseError,
    InvalidRequestError : InvalidRequestError,
    MethodNotFoundError : MethodNotFoundError,
    InvalidParamsError  : InvalidParamsError  
};

})();